...
runname_base = 'test100_LTT9779b_'
...
plt.savefig('./%s/IRAC_spectrum_summary_plot.pdf' % runname)

fits.writeto('./%s/%s_bestfit_spectrum.fits' % (runnamen,filetype), np.vstack((w, spec)), overwrite=True)
fits.writeto('./%s/IRAC_MCspectra_sorted.fits' % runname, np.vstack((w, mc_spec)), overwrite=True)
fits.writeto('./%s/IRAC_MCspectra_unsorted.fits' % runname, np.vstack((w, mc_spec0)), overwrite=True)
...
tools.legc(leg4)
ax4.set_yticklabels([])
fig1.savefig('./%s/IRAC_2d-contributions_summary_plot.pdf' % runname)
fig2.savefig('./%s/IRAC_thermal_profile_summary_plot.pdf' % runname)


# Posterior plots
##############################
prefix = runname + '/IRAC_'
...
plt.savefig('./%s/IRAC_1d_marg.pdf' % runname)
...
os.system('python multinest_marginals.py %s/IRAC_' % runname)
