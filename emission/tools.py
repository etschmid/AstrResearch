...
def drawRectangle(x,y,width,height,**kw):
    ...
    ax.add_patch(p)
    if 'dodraw' in kw and kw['dodraw']: plt.draw()
...
def hparams(params, nbins=10, figsize=[15,10], density=False, normed=False,newfig=True, labs=None, cumulative=False, minorticks=True, plotconf=.683, plotmid=True, color=None):
    ...
    # 2020-03-21 IJMC: Now works in latest Python (getfullargspec)
    from numpy import sqrt
    ...
    if 'density' in getfullargspec(hist)[0]:
...
def sample_2dcdf(cdf, x, y, nsamp=1, verbose=False):
    ...
    # 2020-05-04 IJMC: Fixed indexing issue (Py3?)
    ...
    while nsampled < nsamp:
        ...
        # Accept or reject the candidate sample:
        if prob[0:-1,0:-1][ind_x, ind_y] > accept_prob[ii]:
            ret1[nsampled] = sampx[ii]
            ret2[nsampled] = sampy[ii]
            nsampled += 1
        ii += 1
...
def printtriforce(n, printit=True):
    a = ' /\\ ', '/__\\'
    ret = []
    for ii in range(n):
        spaces = ((n-ii-1)*2)*' '
        newrows = spaces+a[0]*(ii+1), spaces+a[1]*(ii+1)
        ret += newrows
        if printit:
            print( newrows[0])
            print( newrows[1])
...
def pascal(n):
    if n<=1:
        ...
        ret = [[1],[1,1]]
        for ii in range(n-2):
            vec1 = np.concatenate((ret[-1], [0]))
            vec2 = np.concatenate(([0], ret[-1]))
            ret.append(list(vec1+vec2))
    return ret
