...
paramstxt.append('    ln Z = %.1f +- %.1f' % (s['global evidence'], s['global evidence error']))
...
if dim2:
	...
	for i in range(n_params):
		...
		for j in range(i):
			...
			#plt.close()
	txt_ax = plt.subplot(n_params,n_params, (n_params**2 - n_params + 1))#, position=[.02, .02, .1, .1])
	textfig(paramstxt, ax=txt_ax, fig=plt.gcf(), fontsize=12)
